...
| P9 — وكيل صوتي سريع ومتفاعل (وكيل خاص بالمحادثة الصوتية) | ✅ مُنجَز — نظام صوتي مخصص + سلسلة Flash بلا تفكير + ردّ بصوتٍ واحد بلا تقطيع + تنقية «نصوص وأرقام فقط» + تسريع STT (وثّق في `VOICE_AGENT_LATENCY.md`) |
| P10 — تنظيف الواجهة الأمامية (واجهات فقط) | ✅ مُنجَز — حذف زر الصوت من حقل الكتابة + قائمة خيارات المهام عمودية (أيقونة+نص) + إخفاء سطر `edge-tts` من الرئيسية + إخفاء اسم «opencode» من كل الواجهات والإعدادات والردود (وثّق في `UI_CLEANUP.md`) |
| P11 — محرك البحث (شريط الرئيسية = متصفح ويب عند الاتصال، FLOW = باحث يوتيوب، بنية جاهزة لـ IG/TikTok) | 📋 الخطة موثَّقة — جاهزة للتنفيذ عند الطلب، مرفقة خياراتها المعتمدة وعرضها الصارم وملفاتها الجديدة/المعدَّلة بدقة (وثّق في `SEARCH_ENGINE.md`) |
